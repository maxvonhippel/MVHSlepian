...
gmt gmtset FONT_LABEL 10 FONT_ANNOT_PRIMARY 10 MAP_LABEL_OFFSET 0.2 PS_PAGE_ORIENTATION portrait MAP_GRID_PEN_PRIMARY 0.5p COLOR_NAN 255 FORMAT_GEO_MAP +ddd:mm:ss MAP_GRID_PEN_PRIMARY 0.25p,gray
...
R=`gmt gmtinfo -h2 -I1/1 $input1b`
awk '{print $1, $2, $3}' $input1b | gmt xyz2grd -h2 -G$intermediaryoutput -I1 $R -V
gmt grdimage $intermediaryoutput $projJ1 $projR1 -Cslepfuncs_iceland.cpt -E150 -Bg10a10Norn/a5g5WEsN -K > $output1
gmt pscoast -J -R -A5000 -Dl -W0.5p,$coastscolor -O -K >> $output1
gmt psxy $input1a -J -R -W0.5p,$dashedcolor,-- -O -K >> $output1
gmt psxy $input2a -J -R -W0.5p,$dashedcolor,-- -O -K >> $output1
...
gmt psscale -Cslepfuncs_iceland.cpt -Dx5c/-0.25c+w10c/0.5c+jTC+h -B20:"surface density change (cm water equivalent)": -O >> $output1
